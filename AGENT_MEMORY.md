...
| 移动端导航 | `.nav` / `.topbar-nav` 移动端（≤680px）**强制换行到第二行左对齐**，GitHub+分享保持右上（course-planner 曾在 2026-08-13 被误改回 `.top-mid`，已于 2026-08-19 修回左上对齐）。 |
...
- 📌 **历史插曲**：2026-08-13 远程提交 `7a8e857`（作者 HairuoLiu，原意"课程抽屉加 outline.uwaterloo.ca 按课号查历年大纲按钮"）合并时把顶栏误改回含 `.top-mid` 的旧结构、移动端未左对齐。已于 **2026-08-19 重新修回左上对齐**（删除 burger / `.top-mid` / `.navmask` / `.mback`，恢复 `.nav` 直接子元素 + 移动端 `order:3` 规则，并保留 8/13 的 outline 按钮功能）。
...
- ✅ **顶栏规范已修复（2026-08-19）**：course-planner 已恢复移动端导航左上对齐（删除 burger / `.top-mid`，`.nav` 为 `.top .wrap` 直接子元素 + 移动端 `order:3` 规则），与 §4 第 5 条一致。无需未来 agent 再处理。
